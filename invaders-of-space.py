...
def update():  # Pygame Zero update function
    ...
    if gameStatus == 1:
        if player.status < 30 and len(aliens) > 0:  # Is the game still active?
            ...
            updatePoints()
            if moveCounter == 0:
                updateAliens()  # Aliens move, their movement speed increases each level
            moveCounter += 1
            ...
            if keyboard.RETURN:
                if player.lives > 0:  # Player respawns and the game continues
                    ...
                    if len(aliens) == 0:  # Level has been cleared
                        ...
                        boss.active = False
                        bossJuke = math.floor(
                            450 / level)  # Reduce this number as the levels increase, this keeps the chance of the boss pulling a juke fairly consistent instead of the chances decreasing each level
...
def readHighScore():
    ...
    if 1000 <= score < 10000:
        scoreSpacing = "                     "
    if 10000 <= score < 100000:
        scoreSpacing = "                   "
    if 100000 <= score < 1000000:
        scoreSpacing = "                 "
    if score >= 1000000:
        scoreSpacing = "               "
...
def drawHighScore():
    ...
    screen.draw.text("TOP SCORES", midtop=(400, 30), owidth=0.5, ocolor=(255, 255, 255), color=(0, 64, 255),
                     fontsize=60)
    screen.draw.text("LEVEL    SCORE           NAME", topleft=(10, 90), owidth=0.5, ocolor=(255, 255, 255),
                     color=(0, 64, 255), fontsize=60)
    for line in highScore:
        if y < 400:
            screen.draw.text(line, topleft=(10, 150 + y), owidth=0.5, ocolor=(0, 0, 255), color=(255, 255, 0),
                             fontsize=50)
            y += 50
...
def pointsListCleanup(l):
    # Needed a separate function for cleaning up the pointsPopup list as the identical position
    # in the pointsYPOS list needs to be deleted at the same time as a value in pointsPopup
    global pointsYPOS
    newList = []
    for i in range(len(l)):
        if l[i].status == 0:
...
def checkPowerUpHit(p):  # Has the player collected a power up?
    global score, player
    if player.collidepoint((powerUps[p].x, powerUps[
        p].y)) and player.status == 0:  # Player can only collect a power up if their ship isn't currently exploding
        powerUps[p].status = 1
        score += powerUpPoints
        sounds.powerup.play()
...
def checkBigLaserHit(l):
    # The Big Laser doesn't collide with the base segments and doesn't decay if it
    # hits an alien. It continues until it has reached the top of the screen.
    # Aliens don't spawn a power up when hit with the Big Laser
    global score, boss
    for a in range(len(aliens)):
...
def createPoints(t, x, y):
    # Create a pointsPopup at the location that the alien/boss ship was hit by a laser/Big Laser,
    # track the initial Y position of this popup which will be used to check when to delete it from its list
...
def updateBoss():  # Move the boss or check to see if the boss spawns
    ...
    if boss.active:
        ...
        # Boss has reached the bottom of the screen, player doesn't immediately
        # game over like they do with an alien reaching the bottom
        if boss.y > 500:
            sounds.explosion.play()
            player.status = 1
            boss.active = False
    ...
    # the current level number, if that is even possible
        ...
        if randint(0, (1200-(level*50))) == 0: 
            ...
            if boss.spawnPoint == 2:
                ...
                # Choose a random direction of left or right since this spawns boss in the middle of the stage
                boss.direction = randint(0, 1)
